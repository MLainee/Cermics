 ...
 * OPL 12.8.0.0 Model
 * Author: herve
 * Creation Date: 24 sept. 2018 at 11:46:47
 *********************************************/
...
{Crew} crews_temp = {};
 execute{
// 	var f=new IloOplInputFile("Instances/toy_instance.csv"); 
...
/*Objective function */
 minimize sum(f in flights) (y[f]) + sum(fg in reverse_arcs_crew) (fg.c*r[fg])
 + sum(m in maints) m.c * (dt_m[m] - m.tini) + sum(f in flights)f.c_del*gamma[f] 
 + sum(f in flights)sum(fprime in alternative[f])c_rsc[f.id][fprime]*pr[f.id][fprime] 
 + sum(f in flights)f.c_roc*nr[f] + sum(fg in swaps)fg.c*x[fg];
...
// 	var f=new IloOplOutputFile("Outputs/export_05-08-2018_2H.csv");
    var f=new IloOplOutputFile("Outputs/export_400_20_1H.csv");
//    var f=new IloOplOutputFile("Outputs/export_toy_instance.csv");
    ...
    f.writeln("Nombre de recours a un equipage de reserve : ", nb_reserve_crew);
    f.close();   	
 } 
